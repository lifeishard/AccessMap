...
#define MWIDTH			1//04 /* Width of menu window */
 ...
 */
#define FONT_CACHE_SIZE_FOR_ZEROS 4000
#define FONT_CACHE_SIZE_FOR_ROTATED 4000
...
int didMultiThreadingWork = XInitThreads();
...
void LEFTdrawtext(float xc, float yc, const std::string& str_text, float boundx, float boundy) {
    // Need a C-string to call the low-level (X11 or win32) apis.
    const char* text = str_text.c_str();  
    int text_byte_length = strlen(text);
    float angle = (float) PI * gl_state.currentfontrotation / 180;
...
void MYhandle_zoom_in(float x, float y) {
    //make xright - xleft = 0.6 of the original distance 
    trans_coord.xleft = x - (x - trans_coord.xleft) / (float) ZOOM_FACTOR;
    trans_coord.xright = x + (trans_coord.xright - x) / (float) ZOOM_FACTOR;
    //make ybot - ytop = 0.6 of the original distance
...
void MYhandle_zoom_out(float x, float y) {
    //restore the original distances before previous zoom in
    trans_coord.xleft = x - (x - trans_coord.xleft)*((float) ZOOM_FACTOR);
    trans_coord.xright = x + (trans_coord.xright - x)*((float) ZOOM_FACTOR);
    trans_coord.ytop = y - (y - trans_coord.ytop)*((float) ZOOM_FACTOR);
...
void MYproceed(void (*drawscreen) (void)) {
    (void) drawscreen;
    gl_state.ProceedPressed = true;
}
