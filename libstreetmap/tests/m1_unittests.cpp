...
TEST_FIXTURE(Map_Fixture, street_name_functionality) {
    ...
    // Add new unit tests
    // This test converts a random intersection to a vector of segments, 
    //then converts segments to streets, checks that the initial
    // intersection ID exists on every street that is outputted.
    // This test will verify the connectivity of our API by cycling through many 
    //classes and functions
    TEST_FIXTURE(Map_Fixture, intersect_to_seg_to_street_to_intersect) {
        for(unsigned loop = 0; loop < 10; loop++){ // Loops to check 10 times
            // Generates random intersect ID
            int64_t testInput = rand_intersection(rng);
            // Generates some street segments
            vector<int64_t> tempSegs = allIntersects[testInput].getStreetSegs();
            vector<vector<int64_t>> tempStreets;

            //Converts those segments into streets, also puts every intersection
            //on that street into another vector
            for (unsigned i = 0; i < tempSegs.size(); i++) { 
                tempSegs[i] = allStreetSegs[tempSegs[i]].getStreet();
                tempStreets.push_back(allStreets[tempSegs[i]].getIntersects());
            }
            unsigned checkSuccess;

            // Defaults success to zero, if every street has the intersection on
            //it, then test success
            for(unsigned i = 0; i < tempStreets.size(); i++) { 
                checkSuccess = 0;
                for(unsigned j = 0; j < tempStreets[i].size(); j++) {
                    if (tempStreets[i][j] == testInput) { 
                        checkSuccess = 1;
                    }
                }
            }
            CHECK_EQUAL(1, checkSuccess);
        }
    }
    //this unit test implements the the function which is not tested
    //by the previous unit tests above
    //it uses the find_intersection_street_names(id) and uses our api to
    //find a vector of streets connect to it then searches inside the streets
    //to find the original intersection, it repeats this for 100 random 
    //intersections each time
    TEST_FIXTURE(Map_Fixture, find_intersection_street_names_functionality) {
        int64_t testInput ;
        vector<string> tempStreets ;
        vector<int64_t> tempIntersects;
        for(unsigned i = 0; i < 100;i++) {
            testInput = rand_intersection(rng);
            tempStreets = find_intersection_street_names(testInput);

            char f = 0;
            for (unsigned j = 0; j < tempStreets.size(); j++) {
                tempIntersects = allStreets[
                        StreetNametoID[tempStreets[j]]].getIntersects();
                if(find(tempIntersects.begin(),tempIntersects.end(),
                        testInput) != tempIntersects.end()) {
                    f = 1;
                    break;
                }
            }
    ...
    //this unit test computes the speed of the last function
    //find_closest_point_of_interest by using random intersection
    //locations as input
    TEST_FIXTURE(Map_Fixture, timing_analysis) {  
        UNITTEST_TIME_CONSTRAINT(5000);
        int64_t testInput ;
        string resultName;
        for(unsigned i = 0; i < 1000;i++) {
            testInput = rand_intersection(rng);
            resultName = find_closest_point_of_interest(
                    allIntersects[testInput].getLocation());
        }
    }
}
